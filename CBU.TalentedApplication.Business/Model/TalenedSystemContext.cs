...
public partial class TalenedSystemContext : DbContext
{
    ...
    public virtual DbSet<EvaluatorExamValue> EvaluatorExamValues { get; set; }
...
#warning To protect potentially sensitive information in your connection string, you should move it out of source code. You can avoid scaffolding the connection string by using the Name= syntax to read it from configuration - see https://go.microsoft.com/fwlink/?linkid=2131148. For more guidance on storing connection strings, see https://go.microsoft.com/fwlink/?LinkId=723263.
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<EvaluatorExamValue>(entity =>
        {
            entity.ToTable("EvaluatorExamValue");

            entity.HasOne(d => d.Applicant).WithMany(p => p.EvaluatorExamValues)
                .HasForeignKey(d => d.ApplicantId)
                .OnDelete(DeleteBehavior.ClientSetNull)
                .HasConstraintName("FK_EvaluatorExamValue_Applicant");

            entity.HasOne(d => d.Evaluator).WithMany(p => p.EvaluatorExamValues)
                .HasForeignKey(d => d.EvaluatorId)
                .OnDelete(DeleteBehavior.ClientSetNull)
                .HasConstraintName("FK_EvaluatorExamValue_Evaluator");

            entity.HasOne(d => d.Exam).WithMany(p => p.EvaluatorExamValues)
                .HasForeignKey(d => d.ExamId)
                .OnDelete(DeleteBehavior.ClientSetNull)
                .HasConstraintName("FK_EvaluatorExamValue_EvaluatorExamValue");
        });
